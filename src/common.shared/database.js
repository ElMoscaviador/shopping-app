...
function fetchProductsInCategory(category) {
  const categoryItems = products.filter((item) => item.category === category);
  return categoryItems;
}

function fetchSingleProduct(sku) {
  const specificItem = products.find((item) => item.sku === sku);
  return specificItem;
}

export { categories, products, fetchProductsInCategory, fetchSingleProduct };
